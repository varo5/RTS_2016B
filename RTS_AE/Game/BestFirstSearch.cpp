...
aeBstFSPathFinder::aeBstFSPathFinder()
{
	m_n = NULL;
	m_nodegrid = NULL;
}

aeBstFSPathFinder::aeBstFSPathFinder(aeTiledMap * pMap) : aePathFinder(pMap)
{
	m_n = NULL;
	m_nodegrid = NULL;
	Init(pMap);
...
int aeBstFSPathFinder::Init(aeBaseClass * pParent)
{
	m_pParent = pParent;
	if (m_nodegrid != NULL)
	{
...
void aeBstFSPathFinder::Destroy()
{
	if (m_nodegrid != NULL)
	{
		for (int32 i = 0; i < ((aeTiledMap*)m_pParent)->GetMapSize(); i++)
...
void aeBstFSPathFinder::Update(float fDelta)
{
	if (!m_bDebug)
	{
...
void aeBstFSPathFinder::Render(aeRenderer * pRenderer)
{
	if (m_bDebug)
	{
...
void aeBstFSPathFinder::VisitGridNode(int32 x, int32 y)
{
	...
	aeVector2 TempVector = aeVector2{ (float)m_end->m_x - m_nodegrid[x][y].m_x , (float)m_end->m_y - m_nodegrid[x][y].m_y };
...
void aeBstFSPathFinder::SetFinalNode(int32 x, int32 y)
{
	m_end = &m_nodegrid[x][y];
}

WalkStateID aeBstFSPathFinder::GiveAStep()
{
	if (m_open.size() > 0)
