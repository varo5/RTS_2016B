...
aeDFSPathFinder::aeDFSPathFinder()
{
	m_n = NULL;
	m_nodegrid = NULL;
}

aeDFSPathFinder::aeDFSPathFinder(aeTiledMap * pMap) : aePathFinder(pMap)
{
	m_n = NULL;
	m_nodegrid = NULL;
	Init(pMap);
...
int aeDFSPathFinder::Init(aeBaseClass * pParent)
{
	m_pParent = pParent;
	if (m_nodegrid != NULL)
	{
...
void aeDFSPathFinder::Update(float fDelta)
{
	if (!m_bDebug)
	{
...
void aeDFSPathFinder::Render(aeRenderer * pRenderer)
{
	if (m_bDebug)
	{
...
void aeDFSPathFinder::VisitGridNode(int32 x, int32 y)
{
...
void aeDFSPathFinder::SetFinalNode(int32 x, int32 y)
{
	m_end = &m_nodegrid[x][y];
}

WalkStateID aeDFSPathFinder::GiveAStep()
{
	if (m_open.size() > 0)
	{//Hay objetos, por lo que podemos seguir calculando una ruta
		m_n = (aeMapTileNode*)m_open.top();	//Obtenemos el nodo actual para chequeos
